...
\dayentry{2026-01-05}{Rings}
\begin{theorem}
	If $a \neq 0_R$ and $ab = ac$ in an integral domain $R$, then $b = c$.
\end{theorem}
\begin{proof}
	If $ab = ac$, then $ab - ac = 0_R$. Thus, $a(b - c) = 0_R$. As $R$ is an integral domain and $a \neq 0_R$, it must be that $b-c = 0_R$. If $b - c \neq 0_R$, then $a$ would be a zero divisor contradicting the zero-product property of integral domains. Thus $ b =c $. 
\end{proof}

\dayentry{2026-01-06}{Rings}
\begin{theorem}
	Every field is an integral domain. 
\end{theorem} 	
\begin{proof}
	To show that a field $F$ is an integral domain, it suffices to show that $F$ satisfies the zero product property. Take any $a, b \in F$ such that $ab = 0_F$. We must show that $a = 0_F$ or $b = 0_F$. Assume that $ab = 0_F$. Without loss of generality, if $b = 0_F$, then we are done, so assume that $b \neq 0_F$. As $F$ is a field, $b$ is a unit in $F$. By definition of a unit, we have that 
	\begin{align*}
